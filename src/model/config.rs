...
impl Config {
    /// Standard config path: `$XDG_CONFIG_HOME/statusline/config.toml`,
    /// falling back to `$HOME/.config/statusline/config.toml`.
    pub fn default_path() -> Option<PathBuf> {
        let base = std::env::var_os("XDG_CONFIG_HOME")
            .map(PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .or_else(|| std::env::var_os("HOME").map(|h| PathBuf::from(h).join(".config")))?;
        Some(base.join("statusline").join("config.toml"))
    }
