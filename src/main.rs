...
fn run_config(cli: &Cli) -> ExitCode {
    ...
    {
        let _ = cli;
        eprintln!("statusline: built without the `tui` feature — configurator unavailable.");
        eprintln!("Edit the TOML config directly (see `statusline init --print`).");
        ExitCode::FAILURE
    }
}
...
fn run_init(cli: &Cli, force: bool, print: bool) -> ExitCode {
    ...
    if print {
        ...
        let path: PathBuf = match cli.config.clone().or_else(Config::default_path) {
            Some(p) => p,
            None => {
                eprintln!("statusline: could not determine a config path (set $HOME or --config).");
                return ExitCode::FAILURE;
            }
        };
        if path.exists() && !force {
            eprintln!(
                "statusline: {} already exists (use --force to overwrite).",
                path.display()
            );
            return ExitCode::FAILURE;
        }
        match cfg.save(&path) {
            Ok(()) => {
                eprintln!("statusline: wrote default config to {}", path.display());
                ExitCode::SUCCESS
            }
            Err(e) => {
                eprintln!("statusline: {e}");
